...
function transformToLis(obj) {
  const newList = Object.keys(obj).map(item => `<li>${item}: ${obj[item]}</li>`);
  return newList;
}
...
const count = (target, input) => {
  let sum = 0;
  input.map(value => value.map(item => {
    if (item === target) {
      sum++;
    }
  }))
...
const totalSum = (input) => {
  let sum = input.reduce((accumulator, currentValue) => accumulator + currentValue.reduce((accumulator, currentNum) => accumulator + currentNum, 0), 0);
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(value => { return value.filter(value => typeof value === 'number' && value % 5 === 0).map(num => Math.pow(2, num)) })
};
...
let findMaleAndFemale = (data) => {
  const newArr = data.filter(item => item.gender === 'male' || item.gender === 'female').map(element => element.name).join(' and ');
  return newArr;
