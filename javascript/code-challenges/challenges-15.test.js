...
const toTitleCase = (arr) => {
  const newArr = arr.map(string => string.charAt(0).toUpperCase() + string.slice(1));
  return newArr;
};
...
let biggerThanLuke = (arr) => {
  const greaterMass = arr.filter(character => character.mass > parseInt(arr[0].mass)).map(character => character.name).join(' - ');
  return greaterMass;
};
...
const sortBy = (property, arr) => {
  return arr.sort((a, b) => {
    // (a.name > b.name) ? 1 : (a.name === b.name) ? ((a.price > b.price) ? 1 : -1) : -1);
